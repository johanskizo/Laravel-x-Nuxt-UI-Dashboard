<script setup lang="ts">
	/**
	 * ============================================================================
	 * User Management List Page (TypeScript Script Setup)
	 * ============================================================================
	 * Displays a paginated, filterable, and bulk‑action enabled table of users.
	 *
	 * Features:
	 * - Server‑side pagination, sorting, search, and status filter
	 * - Row selection with select‑all and bulk delete support
	 * - Per‑row action buttons (edit, delete) gated by permissions
	 * - Status badges, role badges, and avatar display for each user
	 * - Responsive table and pagination controls using Nuxt UI components
	 *
	 * Data Flow:
	 * 1. On mount → `fetchData()` loads initial user list from `/user/data`
	 * 2. User interacts (sort / paginate / filter / search) → watchers trigger `fetchData()`
	 * 3. User selects rows → `selectedItems` reflects single/bulk selection
	 * 4. Bulk or single delete → `executeDelete()` posts to `/user/bulk-delete`
	 * 5. Success → table is refreshed and selection reset
	 * ============================================================================
	 */
	...
	// Resolving Nuxt UI components for use in render function (h)
	...
	async function fetchData() {
		...
		try {
			const { data: res } = await instance.get('/user/data', {
				params: {
	...
	async function executeDelete() {
		...
		try {
			...
			await instance.post('/user/bulk-delete', { ids });
	...
	watch(
		[sorting, () => pagination.value.perPage, statusFilter],
		() => {
	...
	const columns: TableColumn<any>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				if (roles.length === 0) {
					return h('span', { class: 'text-mute text-xs italic' }, t('No Role'));
				}
				return h(
					'div',
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				// Konversi ke number jika data dari API berupa string
	...
	if (can('User.edit') || can('User.delete')) {
		columns.push({
			id: 'action',
			header: t('Action'),
			cell: ({ row }) => {
					...
					[
						...
						can('User.edit')
							? h(UButton, {
									...
									variant: 'ghost',
									color: 'primary',
									onClick: () => router.push(`/module/user/edit/${row.original.id}`)
								})
							: null,
				...
				); // Filter untuk menghapus nilai null
			}
		});
	}
</script>
			...
			<div class="space-y-4 p-4">
				<div class="flex flex-col items-center justify-between gap-2 sm:flex-row sm:gap-4">
					...
					<div class="flex flex-wrap justify-center gap-2 sm:justify-end">
						<UInput v-model="searchQuery" :placeholder="t('Search...')" icon="i-lucide-search" />
						<USelect
