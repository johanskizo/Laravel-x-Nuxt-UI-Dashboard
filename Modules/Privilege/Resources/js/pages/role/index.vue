<script setup lang="ts">
	/**
	 * ============================================================================
	 * Role Management List Component (TypeScript Script Setup)
	 * ============================================================================
	 * Displays a paginated, filterable table of roles.
	 *
	 * Features:
	 * - Pagination, sorting, search
	 * - Row/item selection with bulk action
	 * - Action buttons (Users, Permissions, edit, delete)
	 * - Status badges
	 *
	 * Data Flow:
	...
	async function fetchData() {
		...
		try {
			const { data: res } = await instance.get('/privilege/role/data', {
				params: {
	...
	async function executeDelete() {
		...
		try {
			...
			await instance.post('/privilege/role/bulk-delete', { ids });
		...
		},
		{
			...
			cell: ({ row }) => {
				return h(
					'span',
					{
						class: row.original.users_count > 0 ? '' : 'italic text-muted'
					},
					row.original.users_count > 0 ? `${row.original.users_count} ${t('Users')}` : t('No Users')
				);
			}
		},
		{
			...
			cell: ({ row }) => {
				return h(
					'span',
					{
						class: row.original.permissions_count > 0 ? '' : 'italic text-muted'
					},
					row.original.permissions_count > 0
						? `${row.original.permissions_count} ${t('Permissions')}`
						: t('No Permissions')
				);
			}
		}
	...
	if (
		can('Privilege.role.user.edit') ||
		can('Privilege.role.permission.edit') ||
		can('Privilege.role.edit') ||
		can('Privilege.role.delete')
	) {
		columns.push({
			id: 'action',
			header: t('Action'),
							...
							? h(
									...
									() =>
										h(UButton, {
											...
											variant: 'ghost',
											color: 'primary',
											onClick: () => router.push(`/module/privilege/role/user/${row.original.id}`)
										})
								)
							: null,

						can('Privilege.role.permission.view')
							? h(
									UTooltip,
									{
										text: t('Permissions'),
									...
									() =>
										h(UButton, {
											...
											variant: 'ghost',
											color: 'primary',
											onClick: () => router.push(`/module/privilege/role/permission/${row.original.id}`)
										})
								)
							: null,
							...
							? h(
									...
									() =>
										h(UButton, {
											...
											variant: 'ghost',
											color: 'primary',
											onClick: () => router.push(`/module/privilege/role/edit/${row.original.id}`)
										})
								)
							: null,
			...
			<div class="space-y-4 p-4">
				<div class="flex flex-col items-center justify-between gap-2 sm:flex-row sm:gap-4">
					<div class="flex flex-wrap justify-center gap-2 sm:justify-start">
						<UButton
							v-if="can('Privilege.role.add')"
							to="/module/privilege/role/add"
							icon="i-lucide-plus"
							color="success"
						>
							{{ t('Add Data') }}
						...
						<UButton
							v-if="can('Privilege.role.delete') && Object.keys(rowSelection).length > 0"
							icon="i-lucide-trash-2"
							color="error"
							variant="soft"
